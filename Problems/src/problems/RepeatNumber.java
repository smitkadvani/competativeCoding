...
public class RepeatNumber {
     ...
     public static ArrayList<Integer> repeatedNumber(final ArrayList<Integer> A) {
         ...
         int len = A.size();
         int sum=0,s_sum=0,max=Integer.MIN_VALUE;
         int a_sum,a_sq_sum;
         int apb,amb;
         int a,b;
        for(int i=0;i<len;i++)
        ...
        a_sum = (max*(max+1))/2;
        a_sq_sum = ((max)*(max+1)*(2*max + 1))/6;
        
        
         System.out.println("" + a_sum);
         System.out.println("" + a_sq_sum);
        amb = sum - a_sum;
        apb = s_sum - a_sq_sum;
